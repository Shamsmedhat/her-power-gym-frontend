...
import { useCreateClient } from "@/hooks/client/use-client";
...
export default function ClientCreateForm({
  ...
  // Hooks
  const { error, isPending, createNewClient } = useCreateClient();
  const clientSchema = useClientsSchema();
    ...
    createNewClient(values, {
      ...
      onError: (err: Error) => {
        if (err.message.includes("not logged in")) {
          toast.error(t("auth-error-create-client"));
        } else {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild className="m-5">
        <Button variant="outline">{t("add-new-client")}</Button>
      </DialogTrigger>
          ...
          >
            ...
            <DialogHeader className="flex flex-row-reverse justify-between items-center sticky top-0 bg-slate-100 z-10 py-4 px-2  border-b">
              <div className="flex flex-col gap-2">
                <DialogTitle>{t("create-new-client")}</DialogTitle>
                <DialogDescription>
                  {t("add-new-client-description")}
                </DialogDescription>
              </div>
