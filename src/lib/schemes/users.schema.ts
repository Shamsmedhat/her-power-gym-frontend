...
export const useUpdatePasswordSchema = () => {
  ...
  return z.object({
    currentPassword: z.string().min(5, { message: t("password-min-length") }),
    newPassword: z.string().min(5, { message: t("password-min-length") }),
  });
};

export type UpdatePasswordFields = z.infer<
